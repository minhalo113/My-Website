...
export async function getServerSideProps(context) {
    ...
    const blogId = slug.substring(0, 24);
    ...
    try {
        ...
        const expectedSlug = blog.slug ? `${blog._id}-${blog.slug}` : blog._id.toString();

        if (slug !== expectedSlug) {
            return {
                redirect: {
                    destination: `/blog/${expectedSlug}`,
                    permanent: true,
                },
            };
        }
        ...
        console.error("SSR Error fetching blog:", error.message);
        return {
            notFound: true,
        };
    }
...
const SingleBlog = ({ serverBlog }) => {
    ...
    useEffect(() => {
        if (serverBlog) {
            setBlog(serverBlog);
            fetchAdjacent(serverBlog._id);
            fetchComments(serverBlog._id);
        }
    }, [serverBlog, fetchAdjacent, fetchComments]);
    ...
    const handleSubmit = async (event) => {
        ...
        setSubmitting(true);
        try {
            const { data } = await api.post(`/blog/${blog._id}/comments`, {
                name: trimmedName,
                email: trimmedEmail,
    ...
    const blogUrl = `https://www.afigureaday.com/blog/${blog ? (blog.slug ? `${blog._id}-${blog.slug}` : blog._id) : ''}`
    const description =
                                        ...
                                        <div className='col'>
                                            ...
                                            <div className='navigations-part'>
                                                <div className='left'>
                                                    {adjacentBlogs.prev ? (
                                                        <>
                                                            <a href={`/blog/${adjacentBlogs.prev._id}${adjacentBlogs.prev.slug ? `-${adjacentBlogs.prev.slug}` : ''}`} className='prev'>
                                                                <i className='icofont-double-left'></i> Previous Blog
                                                            </a>
                                                            <a href={`/blog/${adjacentBlogs.prev._id}${adjacentBlogs.prev.slug ? `-${adjacentBlogs.prev.slug}` : ''}`} className='title'>
                                                ...
                                                <div className='right'>
                                                    {adjacentBlogs.next ? (
                                                        <>
                                                            <a href={`/blog/${adjacentBlogs.next._id}${adjacentBlogs.next.slug ? `-${adjacentBlogs.next.slug}` : ''}`} className='prev'>
                                                                <i className='icofont-double-right'></i> Later Blog
                                                            </a>
                                                            <a href={`/blog/${adjacentBlogs.next._id}${adjacentBlogs.next.slug ? `-${adjacentBlogs.next.slug}` : ''}`} className='title'>
